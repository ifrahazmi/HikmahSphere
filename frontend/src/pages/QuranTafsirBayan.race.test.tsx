...
import { fetchTafsirAyah, fetchTafsirEditions } from '../utils/tafsirBayanApi';
...
jest.mock('../utils/tafsirBayanApi', () => ({
  ...
  fetchTafsirEditions: jest.fn().mockResolvedValue([]),
...
const IBN_KATHIR_BODY = 'ibn kathir tafsir body';

const settingsFor = (edition: TafsirEdition): QuranSettings => ({
  ...DEFAULT_QURAN_SETTINGS,
  tafsirEdition: edition,
  selectedTranslations: ['ur.jalandhry'],
});
...
describe('QuranTafsirBayan edition consistency', () => {
  ...
  beforeEach(() => {
    ...
    (fetchTafsirEditions as jest.Mock).mockResolvedValue(FALLBACK_TAFSIR_EDITIONS);
  });
  ...
  it('keeps the selected edition when a superseded request resolves last', async () => {
    ...
    const ibnKathirRequest = deferred<ReturnType<typeof tafsirAyah>>();

    (fetchTafsirAyah as jest.Mock).mockImplementation((_surah, _ayah, edition) =>
      edition === IBN_KATHIR ? ibnKathirRequest.promise : bayanRequest.promise
    );
    ...
    act(() => {
      swapContext(quranValue(settingsFor(IBN_KATHIR), true));
    });
    await waitFor(() => expect(fetchTafsirAyah).toHaveBeenCalledWith(1, 1, IBN_KATHIR));

    await act(async () => {
      ibnKathirRequest.resolve(tafsirAyah(IBN_KATHIR_BODY));
    });
    expect(await screen.findByText(IBN_KATHIR_BODY)).toBeInTheDocument();
    ...
    expect(screen.getByText(IBN_KATHIR_BODY)).toBeInTheDocument();
